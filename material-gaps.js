...
(function (root) {
  ...
  function gaps(counts, adType, cuts, contract) {
    ...
    // ★ 컷마다 다른 것을 보여 주는 자료(angles='each')만 컷 수와 견준다.
    //
    //   캔은 하나의 단단한 물건이라 한 각도만 있으면 그 각도에 맞춰 컷을 짜면
    //   된다. 「여섯 컷인데 한 장뿐」은 모자란 게 아니다. 공간·사람·앱 화면은
    //   컷마다 다른 곳을 보여 주므로 거기서만 수를 따진다.
    if (cuts) {
      for (i = 0; i < t.materials.length; i++) {
        want = t.materials[i];
        ...
        if ((want.angles || "one") !== "each") {
          // 한 각도로 덮이는 것. 막지 않되 무엇을 정하고 넘어가는지는 적는다.
          notes.push(
            want.kind + " 이(가) " + have + "개이고 컷은 " + cuts +
            "개입니다 — 모자란 것이 아닙니다.\n" +
            "    정할 것 — 가진 각도에 맞춰 컷을 짭니다. 그림(콘티)은 아무 각도로나 " +
