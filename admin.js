    ...
    return '<div class="flow-wrap"><div class="flow-head"><span class="lbl">전체 제작 흐름</span>' +
      ...
      '<div class="flow-track">' + FLOW.map(function (s, i) {
        ...
        var paidAt = (paidStage && i === current)
          ...
          : "";
        // ★ 검수 자리에서는 판의 의견·답변 상태를 머리에 말한다 — 상태는 어느 단계에서나 같은 말만, 이유는 본문에 (09-25 Dan)
        var takeState = paidAt === "review" ? (function () {
          var t = newestTakes(p, s.key).filter(function (f) { return !(f.meta || {}).material; });
          var m = (t[0] || {}).meta || {};
          if (m.spec_fail) return '<em class="ai-update choice">다시 뽑기 차례</em>';
          if (!m.dan_take) return "";
          if (!m.our_reply) return '<em class="ai-update working">의견 답변 준비 중</em>';
          if (!m.settled_at) return '<em class="ai-update choice">답변 확인 차례</em>';
          return '<em class="ai-update choice">다시 뽑기 차례</em>';
        })() : "";
        var paidBadge = takeState ? takeState
          : paidAt === "review"
            ? '<em class="ai-update done">검수 대기</em>'
          : paidAt === "working"
  ...
  function paidBody(p, s, act) {
    ...
    if (at === "start") {
      ...
      return '<div class="lc lc-start"' + tag + ">" +
        ...
        // 영상 단계에서는 준비물을 다시 나열하지 않는다 — 제작 자료 단계에서 만들고 승인이 끝났다(위에 작은 그림으로만).
        //   크레딧과 함께 다시 보이면 「영상 뽑기를 누르면 또 나가나」로 읽힌다 (09-25 Dan 화면)
        (again || s === "video" ? "" : needs) +
        '<div class="lc-row">' +
